...
After viewing the file in Spreadsheet application more detail as shown in [this figure](#pic), firstly, we focus on `Dimension Type` column as it explains different information as follows:
...
This column also has the following Measures:
  ...
  + Job Openings: having its specific Value format
  + Median Base Pay: having its specific Value format
...
We can see that in general there are three formats: those with `$` sign, those without one, and those with `.` at the end each number. So, before we jump to data type change, we have to handle those three. We can use the following codes.
...
```{r}
DF$YoY <- sapply(DF$YoY, function(x) as.numeric(gsub("[%]", "", x)))/100
```
