...
```{r}
year20182019$Metro <- as.factor(year2019$Metro)
```

* `Dimension_Type` column

```{r}
unique(year20182019$Dimension_Type)
```
...
```{r}
year20182019$Dimension_Type <- as.factor(year20182019$Dimension_Type)
```

* `Dimension` column

```{r}
unique(year20182019$Dimension) %>% 
     kable() %>%
     kable_styling() %>%
     scroll_box(width = "500px", height = "200px")
length(unique(year20182019$Dimension))
```

For `Dimension_Type` column, there 135 unique observations. That's a big number! So, we just leave its data type as it is.

* `Measure` column

```{r}
unique(year20182019$Measure)
```

For `Dimension_Type` column, there are 6 unique observations only. So, let's change its data type to factor.

```{r}
year20182019$Measure <- as.factor(year20182019$Measure)
```
...
```{r}
head(year20182019$Date)
tail(year20182019$Date)
```
...
year20182019$Date <- as.character(sapply(year20182019$Date, isString7Chars)) %>% 
     ymd()
```
...
```{r}
head(year20182019$Value)
tail(year20182019$Value)
```

```{r}
unique(year20182019$Value) %>%   
     kable() %>%
     kable_styling() %>%
     scroll_box(width = "500px", height = "200px")
```
...
```{r}
year20182019$Value <- sapply(year20182019$Value, function(x) as.numeric(gsub("[,$]", "", x)))
```
...
```{r}
head(year20182019$YoY)
tail(year20182019$YoY)
```

```{r}
unique(year20182019$YoY) %>%   
     kable() %>%
     kable_styling() %>%
     scroll_box(width = "500px", height = "200px")
```
...
```{r}
year20182019$YoY <- sapply(year20182019$YoY, function(x) as.numeric(gsub("[%]", "", x)))/100
```
...
```{r}
Salary_CompanySize <- year20182019 %>% 
...
```{r}
JobOpening_Sectors <- year20182019 %>% 
...
```{r}
JobOpening_CompanySize <- year20182019 %>% 
...
Oh, crap! There are 88 NAs. First, let's check such column with 88 NAs.
...
This subtable comes from `Timeseries` column in the main dataframe. It's going to have data of median salary and job openings in certain time interval. But, it doesn't have `YoY` column. But, before we decide that we won't use `YoY` column, let's make sure such column when `Dimension_Type == "Timeseries"` is completely NA.
...
Nice! We have 28292 rows and also 28292 NAs in `YoY` column. So, we are 100% sure that we have to remove the column.

As this Time Series subtable comprises median salary and job openings, it will be more convenient if we split it into two smaller dataframe.

* Time Series for Job Openings

```{r}
TimeSeries_JobOpenings <- year20182019 %>% 
...
Whoops! We found some NAs. We don't want this. Let's check the number of NAs then remove all.

```{r}
TimeSeries_JobOpenings %>% 
  is.na() %>% 
  colSums()

print(paste0("The number of dataframe rows is ", nrow(TimeSeries_JobOpenings)))
```

Well, there are 66 NAs only. Not a big number yet if compared to the dataframe rows which is 3432. Therefore, just erase them all.

```{r}
TimeSeries_JobOpenings <- drop_na(TimeSeries_JobOpenings)
```

* Time Series for Median Salary

```{r}
TimeSeries_Salary <- year20182019 %>% 
...
print(paste0("The number of dataframe rows is ", nrow(TimeSeries_Salary)))
```

Great! No NA at all!

And now, we have all transformed the data and are ready for the next section to plot all necessary data.
...
In order to answer this question, we need `Salary_Jobs` dataframe.

```{r}
levels(Salary_Jobs$Area)

for (i in levels(Salary_Jobs$Area)) {
  
  for (j in levels(Salary_Jobs$Job_Title)) {
    a <- Salary_Jobs[Salary_Jobs$Area == i & Salary_Jobs$Job_Title == j,]
    b <- aggregate(formula = Salary ~ Area + Job_Title, data = Atlanta, FUN = mean)
  }
...
aggregate(formula = Salary ~ Area + Job_Title, data = Atlanta, FUN = mean)
